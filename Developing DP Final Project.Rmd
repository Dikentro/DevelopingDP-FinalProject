...
````{r}
countries <- readRDS("C:/Users/diken/OneDrive/Desktop/App-1/counties.rds")
```
...
```{r}
source("C:/Users/diken/OneDrive/Desktop/App-1/helpers.R")
```
...
server <- function(input, output) {
    output$map <- renderPlot({
        ...
        percent_map(var = countries2, color = "brown", legend.title = "USA counties", max = 100, min = 1)
    })
}
``` 
...
Normally up until that moment, you will have corrected possible errors that may have occured during the procedure. If not, after the following command, the final errors to be corrected will appear. 
